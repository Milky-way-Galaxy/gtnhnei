...
async function ValidateAndNotify(page) {
    ...
    if (missing > 0 || changed > 0) {
        ...
        await showConfirmDialog(message, "OK", null, null);
    }
}
export class PageManager {
    ...
    setupEventListeners() {
        ...
        (_a = document.getElementById('nei-link')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', (e) => {
            e.preventDefault();
        ...
        (_b = document.querySelector('[data-action="create-page"]')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => {
            const input = document.querySelector('[data-action="page-name-input"]');
    ...
    generateUniquePageName(baseName) {
    ...
    importPage(model) {
        ...
        if (existingIndex !== -1) {
            ...
            showConfirmDialog(`A page named "${model.name}" already exists. What would you like to do?`, "Create New", "Replace Existing", "Cancel").then(action => {
                if (action === "option1") {
    ...
    saveAndSwitchToPage(pageName, model) {
    ...
    deletePage(pageName) {
        showConfirmDialog(`You are deleting page "${pageName}". This action cannot be undone. Are you sure you want to continue?`, `Delete ${pageName}`, null, `Cancel`).then(action => {
            if (action === "option1") {
                // Remove from storage
