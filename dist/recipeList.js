...
import { Goods, Repository, Fluid } from "./repository.js";
import { UpdateProject, addProjectChangeListener, GetByIid, RecipeModel, RecipeGroupModel, ProductModel, PageModel, DragAndDrop, page, LinkAlgorithm, CopyCurrentPageUrl, DownloadCurrentPage, Search } from "./page.js";
import { voltageTier, formatAmount } from "./utils.js";
import { ShowTooltip } from "./tooltip.js";
import { IconBox } from "./itemIcon.js";
import { ShowDropdown, HideDropdown } from "./dropdown.js";
import { machines, GetSingleBlockMachine, GetParameter } from "./machines.js";
...
export class RecipeList {
    constructor() {
        ...
        this.searchHighlightStyle = document.getElementById('item-icon-search-style');
    ...
    setupActionHandlers() {
        this.actionHandlers.set("delete_product", (obj, event, parent) => {
            if (obj instanceof ProductModel && parent instanceof PageModel && event.type === "click") {
                const index = parent.products.findIndex((p) => p.iid === obj.iid);
                if (index !== -1) {
        ...
        this.actionHandlers.set("crafter_click", (obj, event, parent) => {
            if (obj instanceof RecipeModel && event.type === "click") {
                ...
                const populateDropdown = (container) => {
                    container.innerHTML = `
                        <div class="dropdown-list">
                            Select crafter:
                            ${options.map(goods => {
                        const isSingleblock = recipeType.singleblocks.includes(goods);
                ...
                ShowDropdown(event.target, populateDropdown);
            }
        });
        this.actionHandlers.set("update_amount", (obj, event) => {
            if (obj instanceof ProductModel && event.type === "change") {
                obj.amount = parseFloat(event.target.value) * page.timeScale;
                UpdateProject();
            }
        });
        ...
        this.actionHandlers.set("select_crafter", (obj, event, parent) => {
            ...
            if (obj instanceof RecipeModel && event.type === "click") {
                ...
                if (item) {
                    ...
                    const crafter = Repository.current.GetById(goodsId);
                    if (crafter && ((_a = obj.recipe) === null || _a === void 0 ? void 0 : _a.recipeType.multiblocks.includes(crafter)))
        ...
        this.actionHandlers.set("accept_recipe_change", (obj, event, parent) => {
            if (obj instanceof RecipeModel && event.type === "click") {
                const recipe = Repository.current.GetById(obj.recipeId);
                if (recipe) {
            ...
            if (obj instanceof RecipeModel && event.type === "click") {
                const callback = {
                    onSelectRecipe: (recipe) => {
                        ...
                        obj.recipeId = recipe.id;
                        obj.voltageTier = Math.max((_b = (_a = recipe.gtRecipe) === null || _a === void 0 ? void 0 : _a.voltageTier) !== null && _b !== void 0 ? _b : 0, obj.voltageTier);
                        if (mustResetCrafter) {
                            obj.crafter = undefined;
                            obj.choices = {};
                        }
    ...
    setupGlobalEventListeners() {
        ...
        document.addEventListener("mouseenter", (e) => {
            ...
            if (element) {
                ...
                switch (tooltip) {
                    case "recipe":
                        let obj = (_a = GetByIid(parseInt(element.getAttribute("data-iid")))) === null || _a === void 0 ? void 0 : _a.current;
                        if (obj) {
                            let recipe = Repository.current.GetById(obj.recipeId);
                            let text = `${formatAmount(obj.recipesPerMinute / page.timeScale)} recipes/${page.settings.timeUnit}`;
                            if (recipe === null || recipe === void 0 ? void 0 : recipe.gtRecipe) {
                                ...
                                text = `${obj.parallels} parallels\n` +
                                    ...
                                    `${formatAmount(obj.powerFactor / ((_d = (_c = (_b = obj.recipe) === null || _b === void 0 ? void 0 : _b.gtRecipe) === null || _c === void 0 ? void 0 : _c.amperage) !== null && _d !== void 0 ? _d : 1))}x eu per recipe`;
                            }
                            ShowTooltip(element, {
                                header: (recipe === null || recipe === void 0 ? void 0 : recipe.recipeType.name) + " recipe",
                                text: text,
                                recipe: recipe,
    ...
    setupDragAndDrop() {
        ...
        document.addEventListener("dragstart", (e) => {
            var _a, _b;
            const draggable = (_a = e.target) === null || _a === void 0 ? void 0 : _a.closest("[draggable]");
            if (draggable) {
                draggable.classList.add("dragging");
                (_b = e.dataTransfer) === null || _b === void 0 ? void 0 : _b.setData("text/plain", draggable.getAttribute("data-iid") || "");
            }
        });
        // Handle drag end
        ...
        document.addEventListener("dragover", (e) => {
            var _a;
            e.preventDefault();
            const dropZone = (_a = e.target) === null || _a === void 0 ? void 0 : _a.closest(".recipe-item, .recipe-group, .group-buttons");
            if (dropZone) {
        ...
        document.addEventListener("drop", (e) => {
            ...
            if (!dropZone)
                return;
            dropZone.classList.remove("drag-over");
            const draggedIid = parseInt(((_b = e.dataTransfer) === null || _b === void 0 ? void 0 : _b.getData("text/plain")) || "0");
    ...
    showNeiForRecipeSelection(targetGroup) {
        const callback = {
            onSelectRecipe: (recipe) => {
    ...
    addRecipe(recipe, targetGroup) {
        var _a, _b, _c;
        const minVoltage = (_a = page.settings.minVoltage) !== null && _a !== void 0 ? _a : 0;
        const recipeVoltage = (_c = (_b = recipe.gtRecipe) === null || _b === void 0 ? void 0 : _b.voltageTier) !== null && _c !== void 0 ? _c : 0;
    ...
    renderIoInfo(flow, group) {
        const renderFlowItems = (items, group) => {
        ...
        const renderEnergyItems = (energy) => {
            let totalEnergy = 0;
    ...
    renderRecipeShortInfo(recipe, recipeModel, group) {
        var _a, _b, _c;
        if (recipe === null) {
            return `
                <td colspan="2">
        ...
        let crafter = (_b = (_a = recipeModel.multiblockCrafter) !== null && _a !== void 0 ? _a : recipe.recipeType.singleblocks[recipeModel.voltageTier]) !== null && _b !== void 0 ? _b : recipe.recipeType.defaultCrafter;
        let machineInfo = recipeModel.machineInfo;
        let gtRecipe = recipe.gtRecipe;
        let shortInfoContent = `<span data-tooltip="recipe" data-iid="${recipeModel.iid}">${(_c = crafter === null || crafter === void 0 ? void 0 : crafter.name) !== null && _c !== void 0 ? _c : recipe.recipeType.name}</span>`;
        ...
        if (gtRecipe && gtRecipe.durationTicks > 0) {
            ...
            if (!machineInfo.fixedVoltageTier) {
                ...
                const options = voltageTier
                    ...
                    .map((tier, index) => `<option value="${minTier + index}" ${minTier + index === recipeModel.voltageTier ? 'selected' : ''}>${tier.name}</option>`)
                    .join('');
                shortInfoContent = `
        ...
        if (machineInfo.choices) {
            const choicesHtml = Object.entries(machineInfo.choices).map(([key, choice]) => {
                var _a, _b, _c, _d;
                const currentValue = (_b = (_a = recipeModel.choices[key]) !== null && _a !== void 0 ? _a : choice.min) !== null && _b !== void 0 ? _b : 0;
                let inputHtml = '';
                if (choice.choices) {
                    // Render as dropdown
                    const options = choice.choices.map((option, index) => `<option value="${index}" ${index === currentValue ? 'selected' : ''}>${option}</option>`).join('');
                    inputHtml = `
                    ...
                    inputHtml = `
                        <input type="number" class="machine-choice" 
                            ...
                            data-choice="${key}"
                            value="${currentValue}"
                            min="${(_c = choice.min) !== null && _c !== void 0 ? _c : 0}"
                            max="${(_d = choice.max) !== null && _d !== void 0 ? _d : ''}"
                        >
                    `;
                }
                return `
    ...
    renderRecipe(recipeModel, group, level = 0) {
        let recipe = Repository.current.GetById(recipeModel.recipeId);
        return `
    ...
    renderCollapsedGroup(group, parentGroup, level = 0) {
        return `
    ...
    renderExpandedGroup(group, level = 0) {
        return `
    ...
    renderSettings() {
        const minVoltageOptions = voltageTier.map((tier, index) => `<option value="${index}" ${index === page.settings.minVoltage ? 'selected' : ''}>${tier.name}</option>`).join('');
        return `
            <h2>Settings:</h2>
    ...
    renderProductList() {
        ...
        this.productItemsContainer.innerHTML = `
            ...
            if (!(product instanceof ProductModel))
                return '';
            const goods = Repository.current.GetById(product.goodsId);
            return `
                    <div class="product-item">
                        <item-icon data-id="${goods === null || goods === void 0 ? void 0 : goods.id}" data-action="item_icon_click" data-iid="${page.rootGroup.iid}"></item-icon>
                        ...
                        ${(_a = goods === null || goods === void 0 ? void 0 : goods.name) !== null && _a !== void 0 ? _a : "Unknown product: " + product.goodsId}
